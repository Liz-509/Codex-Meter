...
1. Run `scripts/test-macos.sh`, then `scripts/build-macos.sh` and verify the DMG in `outputs/`.
2. Run `node --check usage-widget.js` separately if you are changing only the shared widget.
