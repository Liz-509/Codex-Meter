...
- On macOS, it compares Codex's saved SSH connection metadata with the system's established SSH connections to determine whether to offer SSH conversation monitoring. Only after the user enables that setting does it reuse the system SSH client to aggregate the same statistics on matching, currently connected hosts; saved but disconnected hosts are not contacted. The command is non-interactive and read-only; remote session files and SSH credentials are not copied into Codex Meter or persisted by it. Enabling this feature can increase refresh latency; disabling it skips remote session collection entirely.
