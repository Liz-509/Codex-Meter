...
- [macOS Apple Silicon installer — Codex Meter v1.4.5](https://github.com/Liz-509/Codex-Meter/releases/download/v1.4.5/Codex-Meter-macOS-arm64-v1.4.5.dmg)
...
- On macOS, watch the current answer and conversation token totals update beside the five-hour allowance, together with context remaining and smooth number transitions.
...
On macOS, the card beside the five-hour allowance uses `token_usage_record.turn_token_usage.total_tokens` for the current answer, falling back to `token_count.info.total_token_usage.total_tokens` for older logs, and refreshes local tasks about every two seconds. The conversation total sums the latest value for every turn in the task, so a new question or context compaction does not reset it. The card also shows context remaining and opens context details. Usage below 60% is Healthy, 60–79% is Attention, 80–89% is High, and 90% or above is Critical. Current-answer usage, conversation usage, context occupancy, today's usage, and account quota are separate metrics. SSH tasks retain the normal refresh cadence; Windows does not yet expose the answer or conversation totals.
...
Extended hosts can also provide `insights.projects`, `insights.tasks`, `contextHealth.sessions`, `forecast.primary`, `forecast.secondary`, and capability flags. Context rows include task/project identifiers, optional `conversationTokens`, `currentTurnId`, `currentTurnTokens`, `currentTurnActive`, `usedTokens`, `maxTokens`, `usedPercent`, `remainingPercent`, `status`, `lastActive`, and `compactions`. `capabilities.contextHealth` enables the context entry; adding `capabilities.currentConversationTokens` makes its main card show the current-answer and conversation totals. Project and task entries may use `projectKind` with `project` or `non_project`; desktop hosts group sessions without a resolvable Git root under “非项目中对话” (non-project conversations). A `partial` refresh payload is used as a cold-start placeholder and does not replace stable history, task, forecast, report, or context-health data after a complete snapshot has arrived. Without `capabilities.extendedInsights`, the shared widget retains its legacy seven-day chart so older embedded hosts do not expose unavailable controls.
